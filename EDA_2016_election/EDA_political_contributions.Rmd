...
tx$donation_level <- factor(tx$donation_level, levels =
                              c("$200 and Under","$200.1 -499" ,"$500-999","$1000-1999","$2000 and over", ordered=T))
...
ggplot(aes(x=cand_nm), data=subset(tx, donation_level =='$200 and Under')) + geom_bar(stat='count') + coord_flip()
```
...
ggplot(aes(x=cand_nm), data=subset(tx, donation_level =='$200.1 -499')) + geom_bar(stat='count') + coord_flip()
```
...
ggplot(aes(x=cand_nm), data=subset(tx, donation_level =='$500-999')) + geom_bar(stat='count') + coord_flip()
```
...
ggplot(aes(x=cand_nm), data=subset(tx, donation_level =='$1000-1999')) + geom_bar(stat='count') + coord_flip()
```
...
ggplot(aes(x=cand_nm), data=subset(tx, donation_level =='$2000 and over')) + geom_bar(stat='count') + coord_flip()
```
...
ggplot(aes(x=number_of_donations), data=tx_contributors) + geom_bar(stat='count') + 
  theme(axis.text.x = element_text(margin = margin(r =10 , b=10, l=10, t=0), angle=90)) + 
  coord_cartesian(xlim=c(0,100)) + scale_y_log10()
...
ggplot(aes(x=contbr_occupation, y=total_employees), data=subset(tx_occupations, total_employees>2000 &
                                                                  !is.na(contbr_occupation) & tx_occupations !='INFORMATION REQUESTED PER BEST EFFORTS' )) + 
  geom_bar(stat='identity') + scale_y_sqrt() + coord_flip()
...
ggplot(aes(x=donation_level, fill=cand_nm), data=subset(tx, cand_nm == 
                                                          c("Trump, Donald J.",'Clinton, Hillary Rodham', "Cruz, Rafael Edward 'Ted'", 'Sanders, Bernard'))) +
  geom_bar(stat='count', position="dodge") + scale_y_sqrt()
...
ggplot(aes(x=cand_nm, y=contb_receipt_amt), data=subset(tx, cand_nm == 
                                                          c("Trump, Donald J.",'Clinton, Hillary Rodham', "Cruz, Rafael Edward 'Ted'", 
                                                            'Sanders, Bernard') & contb_receipt_amt > 0)) + geom_boxplot() + scale_y_log10()

ggplot(aes(x=cand_nm, y=contb_receipt_amt, fill=I("#550000")), data=subset(tx, cand_nm == 
                                                          c("Trump, Donald J.",'Clinton, Hillary Rodham', "Cruz, Rafael Edward 'Ted'", 
                                                            'Sanders, Bernard') & contb_receipt_amt > 0)) +  geom_bar(stat="identity")
...
ggplot(aes(x=number_of_donations, y=mean_donation), data=tx_cities) + geom_point(alpha=1/20, color=I('purple')) +
  coord_cartesian(ylim=c(0,1050)) + scale_x_log10()
...
p1<-ggplot(aes(x=contbr_city, y=hc_support), data=subset(tx_cities, number_of_donations>2500)) + geom_bar(stat='identity') +
  theme(axis.text.x = element_text(angle=90)) +
  ggtitle('Cities Support for Hillary Clinton')

p2 <- ggplot(aes(x=contbr_city, y=dt_support), data=subset(tx_cities, number_of_donations>2500)) + geom_bar(stat='identity') +
  theme(axis.text.x = element_text(angle=90)) +
...
ggplot(aes(x=total_employees, y=contributions_total, color=number_of_donations), data=subset(tx_occupations, !is.na(contbr_occupation))) +
  geom_jitter() + scale_x_log10() + scale_y_log10() + theme_minimal()
```
...
ggplot(aes(x=contbr_city, y=number_of_donations, fill=mean_donation), data=subset(tx_cities, number_of_donations>3500)) +
...
ggplot(aes(x=donation_level, fill=cand_nm), data=subset(tx, 
                                                        cand_nm == c("Trump, Donald J.",'Clinton, Hillary Rodham', 
...
ggplot(aes(x=total_employees, y=contributions_total), data=subset(tx_occupations, !is.na(contbr_occupation))) +
  ...
  ggtitle("Total Number of Employees vs. Total Amount of Donations") + labs(x = 'Total Number of Employees', 
...
ggplot(aes(x=contbr_city, y=number_of_donations, fill=mean_donation), data=subset(tx_cities, number_of_donations>3500)) +
  ...
  labs(fill= "Mean Donation ($)", x = 'City Name', y = "Total Number of Donations", 
